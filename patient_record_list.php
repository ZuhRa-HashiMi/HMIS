...
<?php if (isset($_GET["error"])) { ?>
    <div class="alert alert-danger alert-dismissable">
        <button class="close" data-dismiss="alert" area-hidden="true">&times;</button>
        Could not delete the selected patient!
    </div>
<?php } ?>

<?php if ($totalrows > 0) { // Check if there are patient records to display ?>
    ...
    <ul class="pagination noprint">
        <?php if ($page != 1) { ?>
            <li><a href="patient_record_list.php?page=1">First</a></li>
        <?php } ?>

        <?php if ($page > 1) { ?>
            <li><a href="patient_record_list.php?page=<?php echo $page - 1; ?>">Previous</a></li>
        <?php } ?>

        <?php if ($page < $totalpage) { ?>
            <li><a href="patient_record_list.php?page=<?php echo $page + 1; ?>">Next</a></li>
        <?php } ?>

        <?php if ($page != $totalpage) { ?>
            <li><a href="patient_record_list.php?page=<?php echo $totalpage; ?>">Last</a></li>
        <?php } ?>
    </ul>
    ...
    <ul class="pagination noprint">
        <?php for ($x = 1; $x <= $totalpage; $x++) { ?>
            <li>
                <?php if ($x != $page) { ?>
                    <a href="patient_record_list.php?page=<?php echo $x; ?>"><?php echo $x; ?></a>
                <?php } else { ?>
                    <a href="#"><?php echo $x; ?></a>
                <?php } ?>
            </li>
